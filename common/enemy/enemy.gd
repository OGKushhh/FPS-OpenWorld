...
func _ready() -> void:
        ...
        for col_bone in bone_array:
                col_bone.update_damage.connect(_on_bone_damage)
...
func _on_bone_damage(damage: float, direction: Vector3, hit_zone: int) -> void:
        # Bone damage is already multiplied by the bone's damage_multiplier
        # Just pass it through to get_damage with the hit zone info
        get_damage(damage, direction, hit_zone)
